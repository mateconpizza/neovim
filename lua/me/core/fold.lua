...
function M.text()
  ...
  local lines = '[' .. string.format('%02d', line_count) .. ft_glyph.small_l .. ']'
  ...
  local prefix = ft_glyph.raquo .. ft_glyph.middot .. ft_glyph.middot .. lines
  local suffix = ': '
  ...
  local dashes = string.rep(ft_glyph.middot, dash_count)
...
local parse_markers = function(lines, start_index)
  ...
  for i, line in ipairs(lines) do
    ...
    if start_level ~= nil and tonumber(start_level) ~= 0 then
      markers[#markers + 1] = {
        line = line_number,
        level = tonumber(start_level + 1),
        kind = 'start',
      }
    elseif end_level ~= nil and tonumber(end_level) ~= 0 then
      markers[#markers + 1] = {
        line = line_number,
        level = tonumber(end_level + 1),
        kind = 'end',
      }
